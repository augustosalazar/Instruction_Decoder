
export const REGISTER_BY_BITS = {
    '00000': 'zero',
    '00001': 'at',
    '00010': 'v0',
    '00011': 'v1',
    '00100': 'a0',
    '00101': 'a1',
    '00110': 'a2',
    '00111': 'a3',
    '01000': 't0',
    '01001': 't1',
    '01010': 't2',
    '01011': 't3',
    '01100': 't4',
    '01101': 't5',
    '01110': 't6',
    '01111': 't7',
    '10000': 's0',
    '10001': 's1',
    '10010': 's2',
    '10011': 's3',
    '10100': 's4',
    '10101': 's5',
    '10110': 's6',
    '10111': 's7',
    '11000': 't8',
    '11001': 't9',
    '11010': 'k0',
    '11011': 'k1',
    '11100': 'gp',
    '11101': 'sp',
    '11110': 'fp',
    '11111': 'ra',
} as const;

export const REGISTER_BY_NAME =
    // Invertir de { clave: valor } a { valor: clave }
    Object.fromEntries( Object.entries(REGISTER_BY_BITS).map(([bits, name]) => [name, bits])) as Record<RegisterName, RegisterBits>;

export const REG_ZERO = '00000' as const;
export const REG_RA   = '11111' as const;

export type RegisterBits = keyof typeof REGISTER_BY_BITS
export type RegisterName = typeof REGISTER_BY_BITS[RegisterBits];
